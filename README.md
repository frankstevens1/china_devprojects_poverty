...
### View Notebooks/Code:
- [Preparing the subnational level dataset](dataset_prep.ipynb)
- [Preparing the country level dataset](dataset_prep_iso.ipynb)

- Data Visualization: [Graphs](visuals.ipynb) & [Tables](tables.ipynb)
- [Analysis: Regression Tables & Visualization](stata_do/analysis.do)
...
Where equation 2 is the main equation of interest and *iwipov50<sub>irt</sub>* is the percentage of households in poverty in region *r* in country *i* in year *t*. *CnAid<sub>ir(t-2)</sub>* indicates the presence of a Chinese development project, lagged by 2 years to allow for project completion – based on the average time that projects in the dataset are completed. *ln⁡(Steel<sub>(t-3)</sub>* is the natural logarithm of metric tonnes of steel produced in China lagged by an additional year to allow for domestic over production to translate into the commitment of international development projects. *p<sub>ir</sub>* ⁡is the probability that a region receives a project. The initial level of poverty, *iwipov50<sub>ir(t-1)</sub>*, is included to control for an overall trend in poverty levels. The control is also included in the first stage equation.
